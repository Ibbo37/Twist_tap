...
export default function Admin() {
  ...
  useEffect(() => {
    const fetchUserVideos = async () => {
      setLoading(true);
      try {
        const response = await fetch('http://localhost:5000/api/video/pendingVideo');
        if (!response.ok) throw new Error('Failed to fetch videos');
        const data = await response.json();
        setVideos(data.filter(video => video.status === 'pending'));
      } catch (error) {
  ...
  const handleApprove = async (id) => {
    if (!id) return;
    try {
      const response = await fetch('http://localhost:5000/api/video/approvedVideo', {
        method: 'POST',
  ...
  const handleDecline = async (id) => {
    ...
    if (!reason) return;
    try {
      const response = await fetch('http://localhost:5000/api/video/declinedVideo', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      <ul>
        {videos.filter(video => 
          ...
          <li key={video._id} className="mb-2 cursor-pointer hover:bg-gray-200 p-2 rounded-lg" onClick={() => setSelectedVideo(video)}>
            📽 {video.videoName} ({video.uniqueId})
          </li>
        ))}
      </ul>
    )}
    ...
    {selectedVideo ? (
      <>
        ...
        <video controls className="w-full max-w-[500px] rounded-lg shadow mb-4">
          <source src={selectedVideo.url || selectedVideo.path} type="video/mp4" />
